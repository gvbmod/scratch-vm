// Name: NFCWarp
// ID: alestorenfc
// Description: Allows reading data from NFC (NDEF) devices. Only works in Chrome on Android.
// By: Alestore Games <https://scratch.mit.edu/users/aleb2005/>
// Context: NFC stands for "Near-field communication". Ideally check a real phone in your language to see how they translated it.
// License: MIT

/* generated l10n code */Scratch.translate.setup({"fi":{"_NFC not supported":"lähitiedonsiirtoa ei tueta","_NFC supported?":"tuetaanko lähitiedonsiirtoa?","_NFCWarp":"Lähitiedonsiirto","_Only works in Chrome on Android":"Toimii ainoastaan Chromessa Androidilla.","_read NFC tag":"lue lähitiedonsiirtotunniste"},"it":{"_NFC supported?":"NFC supportato","_NFCWarp":"Sensore di Prossimità (NFC)","_Only works in Chrome on Android":"Funziona soltanto in Chrome per Android","_read NFC tag":"leggi tag NFC"},"ja":{"_NFC not supported":"NFCはサポートされていません","_NFC supported?":"NFCはサポートされているか","_Only works in Chrome on Android":"Android上のChromeでのみで動作","_read NFC tag":"NFCタグを読み取る"},"ko":{"_NFC not supported":"NFC 지원되지 않음","_NFC supported?":"NFC를 지원하는가?","_NFCWarp":"NFC워프","_Only works in Chrome on Android":"Android 크롬 브라우저만 지원함","_read NFC tag":"NFC 태그 읽기"},"nb":{"_NFC not supported":"NFC ikke støttet","_NFC supported?":"NFC støttet?","_NFCWarp":"NFCFordeling","_Only works in Chrome on Android":"Bare fungerer i Chrome på Android","_read NFC tag":"les NFC-taggen"},"nl":{"_NFC not supported":"NFC wordt niet ondersteund","_NFC supported?":"NFC ondersteund?","_Only works in Chrome on Android":"Werkt alleen in Chrome op Android","_read NFC tag":"gegevens van NFC tag"},"ru":{"_NFC not supported":"NFC не поддерживается","_NFC supported?":"Поддерживается NFC?","_Only works in Chrome on Android":"Работает только для Chrome на Android","_read NFC tag":"считывание NFC-метки"},"zh-cn":{"_NFC not supported":"不支持NFC","_NFC supported?":"支持NFC？","_NFCWarp":"NFC","_Only works in Chrome on Android":"只能在安卓的Chrome运行","_read NFC tag":"读取NFC标签"}});/* end generated l10n code */(function (Scratch) {
  "use strict";

  /* globals NDEFReader */

  const extIcon =
    "data:image/png;base64,";
  const blocksIcon =
    "data:image/svg+xml;base64,";

  class NFCWarp {
    getInfo() {
      return {
        id: "alestorenfc",
        name: Scratch.translate("NFCWarp"),
        color1: "#FF4646",
        color2: "#FF0000",
        color3: "#990033",
        menuIconURI: extIcon,
        blockIconURI: blocksIcon,
        blocks: [
          {
            blockType: Scratch.BlockType.LABEL,
            text: Scratch.translate("Only works in Chrome on Android"),
          },
          {
            opcode: "supported",
            blockType: Scratch.BlockType.BOOLEAN,
            text: Scratch.translate("NFC supported?"),
          },
          {
            opcode: "nfcRead",
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate("read NFC tag"),
            disableMonitor: true,
          },
        ],
      };
    }

    supported() {
      return typeof NDEFReader !== "undefined";
    }

    nfcRead() {
      if (!this.supported()) {
        return Scratch.translate("NFC not supported");
      }
      return new Promise((resolve, reject) => {
        const ndef = new NDEFReader();
        ndef
          .scan()
          .then(() => {
            ndef.onreadingerror = (event) => {
              console.log("Reading error", event);
              resolve("Tag not supported");
            };
            ndef.onreading = (evt) => {
              const decoder = new TextDecoder();
              const record = evt.message.records[0];
              console.log("Record type: " + record.recordType);
              console.log("Record encoding: " + record.encoding);
              console.log("Record data: " + decoder.decode(record.data));
              resolve(decoder.decode(record.data));
            };
          })
          .catch((error) => {
            console.log("Scan error", error);
            resolve(`Error: ${error}`);
          });
      });
    }
  }

  Scratch.extensions.register(new NFCWarp());
})(Scratch);
