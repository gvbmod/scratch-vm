/**
In Full Honesty, CREDIT TO: UchiwaFuujinn
He/She was the one that made this extension.
I just patched lots of bugs and things that it had.
For the documentation, its here:
https://github.com/UchiwaFuujinn/scratch3webmidi

The only reason why i picked this extension, was to make a midi player
that atually like plays midi out instead of the note blocks.
*/
...
//this extension is not mine, but i did fix some annoying bugs.
//this function below is a patch to make the midi out duration work.
//but it can also fix other blocks too.

//this function converts plain text numbers into atually recognized as number numbers.
//its kinda odd to explain, but it just converts it into javascript numbers, if possible, else it uses its own value.
//for some reason they never used a number *parser* which caused it to be
//buggy as crap, so many things like variables and stuff will not work
//on inputs. and since the turbowarp compiler uses parsed numbers.
//so I will just override the args and it should fix it for now.
function argsPatch(argsin) {
	var argsout = {};
	for (var arg of Object.keys(argsin)) {
		if (!isNaN(Number(argsin[arg]))) {
			argsout[arg] = Number(argsin[arg]);
		} else {
			argsout[arg] = argsin[arg];
		}
	}
	return argsout;
}
...
//		No alerts, so it will instantly load the webmidi.
//		alert( "Success MIDI!\n", msg );
//		alert( msg );
		console.log(msg);
	}
...
}catch(e){console.error("m_midiin Fail! ",e);};
}
...
function m_noteon(note, vel)
{
	...
	}catch(e){console.error("m_noteon Fail! ",e);};
}
...
*/
	if(mOutputs!=null){
		if(mOutDev<mOutputs.length){
			...
			if(l_output!=null){
				l_output.send([data1,data2,data3], getNowTime());
			}
		}
	}
	}catch(e){console.error("M_MIDIOUT Fail! ",e);};
}
...
*/
	if(mOutputs!=null){
		if(mOutDev<mOutputs.length){
			var l_output=mOutputs[mOutDev];
			if(l_output!=null){
				l_output.send([data1,data2], getNowTime());
			}
		}
	}
	}catch(e){console.error("m_midiout_2byte Fail! ",e);};
}
...
*/
		if(mOutDev<mOutputs.length){
			var l_output=mOutputs[mOutDev];
			if(l_output!=null){
				l_output.send(buf, getNowTime());
			}
		}
	}
	}catch(e){console.error("m_sysexout Fail! ",e);};
}
...
//Gvbvdxx created blocks *********************************************
				
				
				{
					opcode: 'gvbMidiOutNoteBlock',
					text: formatMessage({
						id: 'webmidi.gvbMidiOutNoteBlock',
						default: 'Play note [note] for [length] seconds.',
						description: 'Presses And Releases A Note For A Amount Of Seconds'
					}),
					blockType: BlockType.COMMAND,
					arguments: {
						note: {
				...
				{
					opcode: 'gvbMidiOutInstrument',
					text: formatMessage({
						id: 'webmidi.gvbMidiOutInstrument',
						default: 'Set my instrument to [inst]',
						description: 'Sets the sprites instrument to the channel provided'
					}),
					blockType: BlockType.COMMAND,
					arguments: {
						inst: {
							type: ArgumentType.NUMBER,
							menu: 'midioutinstruments',
							defaultValue: 0
						}
					}
				},
				...
				{
					opcode: 'gvbMidiOutChannel',
					text: formatMessage({
						id: 'webmidi.gvbMidiOutChannel',
						default: 'Set my channel to [channel]',
						description: 'Sets the channel to the number'
					}),
					blockType: BlockType.COMMAND,
					arguments: {
						channel: {
				...
				{
					opcode: 'gvbMidiOutVolume',
					text: formatMessage({
						id: 'webmidi.gvbMidiOutVolume',
						default: 'Set my volume to [volume]%',
						description: 'Sets the volume percentage to the number'
					}),
					blockType: BlockType.COMMAND,
					arguments: {
						volume: {
...
//END Gvbvdxx Created Blocks *****************************************
            ...
            menus: {
				...
				midioutinstruments: {
					acceptReporters: true,
					items: require("./midi.js")
				}
            }
		};
	}
	
	//Gvbvdxx created functions
	
	gvbInitMidiOut (target) {
		if (typeof target.channelNumber == 'undefined') {
			target.channelNumber = 1;
		}
		if (typeof target.instrumentNumber == 'undefined') {
			target.instrumentNumber = 1;
		}
		if (typeof target.midiOutVolume == 'undefined') {
			target.midiOutVolume = 100;
		}
	}
	
	gvbMidiOutNoteBlock (args, util) {
		this.gvbInitMidiOut(util.target);
		var target = util.target;
		var note = Number(args.note);
		var length = Number(args.length);
		this.s_ProgramChange({
			channelnum:target.channelNumber,
			pnumber:target.instrumentNumber
		});
		this.s_Noteon_out_duration({
			notenum:note,
			channelnum:target.channelNumber,
			velo:(target.midiOutVolume/100)*127,
			duration:length*1000
		});
		return new Promise((a) => {
			setTimeout(a,length*1000);
		});
	}
	
	gvbMidiOutInstrument (args, util) {
		this.gvbInitMidiOut(util.target);
		var inst = Number(args.inst);
		var target = util.target;
		target.instrumentNumber = inst;
	}
	
	gvbMidiOutChannel (args, util) {
		this.gvbInitMidiOut(util.target);
		var channel = Number(args.channel);
		var target = util.target;
		target.channelNumber = channel;
	}
	
	gvbMidiOutVolume (args, util) {
		this.gvbInitMidiOut(util.target);
		var volume = Number(args.volume);
		var target = util.target;
		target.midiOutVolume = volume;
	}
...
// GET KEY ON with keynumber
	s_Getkeyonnum(pargs) {
		...
		// Reset alarm_went_off if it is true, and return true
		// otherwise, return false.
		if (mNoteOn[Number(args.ckeynum)] == true) {
			mNoteOn[Number(args.ckeynum)] = false;
			return true;
       }
       return false;
	}
...
// ccnum: Control Change Number
	s_Ccin(pargs) {
		var args = argsPatch(pargs);
		return (mCtlbuf[Number(args.ccnum)]);
	}
...
//NOTE ON
	s_Noteon_out(pargs){
		var args = argsPatch(pargs);
		//console.log("s_Noteon_out",args);
		var chnum = (Number(args.channelnum)&0x0F)-1;
		return m_midiout(0x90+chnum,Number(args.notenum)&0x7F,Number(args.velo)&0x7F);
	}
	
	MIDIOUT_NOTE_ON_DUR (args) {
		var th = this;
		//th.s_Noteon_out(args);
		setTimeout(() => {
			var chnum = (Number(args.channelnum)&0x0F)-1;
			m_midiout(0x80+chnum,Number(args.notenum)&0x7F,Number(args.velo)&0x7F);
		},Number(args.duration));
		var chnum = (args.channelnum&0x0F)-1;
		return m_midiout(0x90+chnum,Number(args.notenum)&0x7F,Number(args.velo)&0x7F);
	}

	s_Noteon_out_duration(pargs){
		var args = argsPatch(pargs);
		//console.log("s_Noteon_out_duration",args);
		var chnum = (Number(args.channelnum)&0x0F)-1;
		var irticks=Number(args.duration); //scratch has this thing where the number will sometimes be a string, to fix this, try to convert the string into a number.
		if(irticks<10) irticks=10;
		var waittime = irticks/mResolution * 60/mTempo*1000;
		setTimeout( function(){m_midiout(0x80+chnum,Number(args.notenum)&0x7F,0x40);}
			,Number(waittime));
		return m_midiout(0x90+chnum,Number(args.notenum)&0x7F,Number(args.velo)&0x7F);
...
//NOTE OFF
	s_Noteoff_out(pargs){
		var args = argsPatch(pargs);
		//console.log("s_Noteoff_out",args);
		var chnum = (Number(args.channelnum)&0x0F)-1;
		return m_midiout(0x80+chnum,Number(args.notenum)&0x7F,Number(args.velo)&0x7F);
	}

//Program Change
	s_ProgramChange(pargs){
		var args = argsPatch(pargs);
		var chnum = (Number(args.channelnum)&0x0F)-1;
		var pgnum = (Number(args.pnumber)&0x7F);
...
//MIDI Output Device Number
	s_OutDevice(pargs){
		var args = argsPatch(pargs);
		if(Number(args.outdev)<1) args.outdev=1;
		else if(Number(args.outdev)>mOutputs.length) args.outdev=mOutputs.length;
		mOutDev=Number(args.outdev)-1;
	}
...
*/
     ...
     */
	s_RestTicks(pargs, util) {
		var args = argsPatch(pargs);
		var iCount = this.mCount;

		if (this._stackTimerNeedsInit(util)) {
            let irticks = Number(args.rticks);
